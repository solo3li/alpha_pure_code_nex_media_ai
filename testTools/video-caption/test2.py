...
def process_direct():
    ...
    file_path = "/home/ubuntu/video-caption/sample2.mp4"
    language = "ar"  # ← حدد اللغة هنا

    try:
        with open(file_path, "rb") as f:
            files = {"video_file": f}
            data = {"language": language}  # ← نضيف اللغة هنا
            headers = {"x-api-key": API_KEY}
            
            r = requests.post(
                f"{API_BASE}/process_direct",
                files=files,
                data=data,
                headers=headers
            )
            ...
            if "video" in content_type:
                with open("result_witarabic.mp4", "wb") as out:
                    out.write(r.content)
                print("📥 Video saved as result2.mp4")
            else:
