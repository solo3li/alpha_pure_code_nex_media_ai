...
def paypal_webhook(request):
    ...
    try:
        ...
        if event_type == "CHECKOUT.ORDER.COMPLETED":
            ...
            payment_id = resource.get("id")  # This is the capture/payment ID
            ...
            print(f"Payment capture completed - Payment ID: {payment_id}, Order ID: {order_id}")
            
            # Check if already processed (check both payment_id and order_id)
            if Payment.objects.filter(payment_id__in=[payment_id, order_id]).exists():
                logger.info(f"Payment already processed (Payment ID: {payment_id} or Order ID: {order_id})")
                return JsonResponse({"status": "already_processed"}, status=200)

            # Extract payment data from the capture webhook
            user_id, plan_name, amount = extract_capture_webhook_data(data)
            
            if user_id and plan_name:
                # Use the payment_id (capture ID) as the primary identifier
                success = process_payment(user_id, plan_name, payment_id, amount)
                if success:
                    logger.info(f"Payment capture completed for Payment ID {payment_id} (Order ID: {order_id}) - subscription created")
                    return JsonResponse({"status": "payment_completed"}, status=200)
                else:
                    logger.error(f"Failed to process payment capture for Payment ID {payment_id}")
            else:
                logger.error(f"Missing data in payment capture webhook for Payment ID {payment_id}")
                
        elif event_type == "PAYMENT.CAPTURE.DECLINED":
            # Payment was declined - log for monitoring
            payment_id = resource.get("id")
            logger.warning(f"Payment capture declined for Payment ID {payment_id}")
...
def extract_capture_webhook_data(webhook_data):
    """Extract payment data from PAYMENT.CAPTURE.COMPLETED webhook"""
    try:
        print(f"Extracting data from capture webhook: {webhook_data}")
        resource = webhook_data.get("resource", {})
        
        # Get amount from resource.amount
        amount_data = resource.get("amount", {})
        amount = float(amount_data.get("value")) if amount_data.get("value") else None
        
        # Get custom_id from resource.custom_id (contains user_id|plan_name)
        custom_id = resource.get("custom_id")
        print(f"Custom ID from capture: {custom_id}")
        
        if custom_id and "|" in custom_id:
            parts = custom_id.split("|", 1)
            user_id = parts[0]
            plan_name = parts[1]
            print(f"Extracted - User ID: {user_id}, Plan: {plan_name}, Amount: {amount}")
            return user_id, plan_name, amount
        
        print("No valid custom_id found in capture webhook")
        return None, None, None

    except Exception as e:
        logger.error(f"Error extracting capture webhook data: {e}")
        return None, None, None


def extract_webhook_data(webhook_data):
    """Extract payment data from ORDER webhook (legacy - keeping for backward compatibility)"""
    try:
        ...
        if not purchase_units:
            print("No purchase units found - this might be a capture webhook")
            # Try to extract from capture webhook format
            return extract_capture_webhook_data(webhook_data)
...
def capture_paypal_payment(request):
    ...
    try:
        ...
        if capture_result['status'] == 'completed':
            ...
            # Get the capture/payment ID from the result
            payment_id = capture_result.get('payment_id') or order_id
            
            success = process_payment(
                user_id=stored_order['user_id'],
                plan_name=stored_order['plan_name'],
                order_id=payment_id,  # Use payment_id if available, otherwise order_id
            ...
            if success:
                ...
                request.session['order_id'] = payment_id
            ...
            if order_details and order_details.get('status') == 'APPROVED':
                ...
                success = process_payment(
                    user_id=stored_order['user_id'],
                    plan_name=stored_order['plan_name'],
                    order_id=order_id,  # Use order_id for manual processing
                    amount=stored_order['amount']
                )
